...
const LineChart = () => {
  ...
  useEffect(() => {
    // Update the chart labels and data separately when viewers change
    if (chartInstance.current) {
      chartInstance.current.data.labels = viewers.map((_, index) => `Date ${index + 1}`);
      chartInstance.current.data.datasets[0].data = viewers;
