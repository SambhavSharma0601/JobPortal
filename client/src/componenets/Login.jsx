...
const Login = () => {
  ...
  //         setTimeout(() => navigate('/userdash', { state: { userName: result.data.user.name } }), 1000);
  //       } else if (result.data.success) {
  //         toast.success('Login successful!', { position: 'top-center' });
  //         // setTimeout(() => navigate('/userdash2'), 1000);
  //         setTimeout(() => navigate('/userdash2', { state: { userName: result.data.user.name } }), 1000);
  //       } else {
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-slate-200" >
      <div className="bg-white p-8 rounded shadow-2xl w-96">
        <h1 className="text-3xl text-blue-500 mb-6 text-center">Job Portal</h1>
